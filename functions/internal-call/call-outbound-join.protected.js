...
const addParticipantToConference = (
  client, conferenceSid, taskSid, to, fromName) => {

  if (to.substring(0, 6) === 'client') {
    return client.taskrouter.workspaces(process.env.TWILIO_WORKSPACE_SID).
      tasks.
      create(
        {
...
exports.handler = async (context, event, callback) => {
  ...
  if (event.StatusCallbackEvent === 'participant-join') {
    ...
    if (call.to.includes('client')) {
      ...
      if (attributes.worker_call_sid ===
        attributes.conference.participants.worker) {
        const { to, fromName } = attributes
        const result = await addParticipantToConference(client, ConferenceSid,
          taskSid, to, fromName)
    ...
    try {
      ...
      if (['assigned', 'pending', 'reserved'].includes(task.assignmentStatus)) {
        await client.taskrouter.workspaces(context.TWILIO_WORKSPACE_SID).
          tasks(taskSid).
          update({
            assignmentStatus: task.assignmentStatus === 'assigned'
              ? 'wrapping'
              : 'canceled',
